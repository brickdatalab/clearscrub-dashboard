...
export default function Companies() {
  ...
  const getStatusBadgeClass = (status: Company['file_status']) => {
    switch (status) {
      case 'completed':
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-start">
        <div className="relative">
          ...
          {showSuggestions && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-6 shadow-lg z-10 max-h-60 overflow-y-auto">
              {filteredSuggestions.map((company) => (
                ...
                >
                  <div>
                    <div className="text-14 font-medium text-gray-900">{company.name}</div>
                    <div className="text-13 text-gray-500">{company.email}</div>
                  </div>
                  <span className={`px-2 py-1 rounded-full text-12 font-medium ${getStatusBadgeClass(company.file_status)}`}>
